...
| Embedded System | Adafruit Circuit Playground Bluefruit | All-in-one MCU, BLE radio, and onboard LIS3DH |
| Firmware | CircuitPython, Adafruit BLE | Sensor-side logic and BLE service |
...
```
.
├── .gitattributes # Enforces consistent line endings (LF)
├── .gitignore # Ignores logs, venv, and system files
├── LICENSE # MIT License
├── README.md
...
│ └── main.py
└── sensor-firmware/
├── code.py # Main CircuitPython logic
...
1. Clone this repository:  
   `git clone https://github.com/StillTyping1/The-CareBeat-Monitor.git`
2. Plug in your board so it appears as a `CIRCUITPY` drive.
3. Copy the contents of `sensor-firmware/` (code.py, settings.toml, lib/) to the `CIRCUITPY` drive.
4. The sensor reboots and begins broadcasting BLE data.
